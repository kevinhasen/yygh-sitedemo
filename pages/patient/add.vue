          ...
          <div class="content-wrapper">
            <el-form :model="patient" label-width="110px" label-position="left" ref="patient" :rules="validateRules">
              ...
              <el-form-item prop="birthdate" label="出生日期：">
                <el-date-picker v-model="patient.birthdate" class="v-date-picker" value-format="yyyy-MM-dd"  type="date" placeholder="选择日期">
                </el-date-picker>
              </el-form-item>
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.$route.query.id) {
        setTimeout(() => {
          this.$refs.selectedShow.presentText = this.patient.param.provinceString + '/' + this.patient.param
            .cityString + '/' + this.patient.param.districtString //"北京市/市辖区/西城区";// 首次手动复制
        }, 1000)
      }
    },
    methods: {
      ...
      fetchDataById(id) {
        patientApi.getById(id).then(response => {
          ...
          //添加默认值
          this.patient.addressSelected = [this.patient.provinceCode, this.patient.cityCode, this.patient
            .districtCode
          ]
        })
      },
